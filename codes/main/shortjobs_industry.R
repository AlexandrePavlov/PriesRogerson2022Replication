...
#drop if time<=1998*4+4 | time>2018*4-2

#can't compare with factors so I need to transform as numeric
#7961 --> 1
#1998*4+4=7996 --> 36
#2018*4-2=8070 --> 110
#hence condition should be to drop if time<=36 | time>110
data_sa <- data_sa[!(as.numeric(data_sa$time) <= 36 | as.numeric(data_sa$time) > 110), ]
...
write.csv(data_sa,file="shortjobs_industry.csv")
...
ggsave('oneqseps_firstlast_industry.pdf')
...
ggsave('hirerate_firstlast_industry.pdf')