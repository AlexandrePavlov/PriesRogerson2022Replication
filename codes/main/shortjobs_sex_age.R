...
data_sa <- read.csv(paste(wd,"qwisa.csv",sep="/"))
...
data_sa <- data_sa[!(as.numeric(data_sa$time) <= 36 | as.numeric(data_sa$time) > 112), ]
...
write.csv(data_sa,file="shortjobs_sex_age.csv")
...
ggsave('oneqseps_firstlast_sex_age.pdf')
...
ggsave('hirerate_firstlast_sex_age.pdf')