...
def calculate_mean_wages():
    ...
    tmatrix = np.array([[1-pf*probmatch, lam+(1-lam)*alph*(1-probg),lam+(1-lam)*alph*(1-probg),lam,lam],            [pf*probmatch, (1-lam)*(1-pkh)*(1-alph),0,0,0],            [0,(1-lam)*(1-alph)*pkh, (1-lam)*(1-alph), 0, 0],            [0,(1-lam)*(1-pkh)*alph*probg, 0, (1-lam)*(1-pkh), 0],            [0,(1-lam)*pkh*alph*probg, (1-lam)*alph*probg, (1-lam)*pkh, (1-lam)]])
    ...
    ave_wg_ten_profile = [w0*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                          w1*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                          w0[-1], w1[-1]]@tendist[1:5,:]/np.sum(tendist[1:5,:],axis=0)
    
    mean_wages_first_qtr = np.sum([w0*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                                    w1*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                                    w0[-1],w1[-1]]@tendist[1:5,:13])/np.sum(tendist[1:5,:13])

    mean_wages_first_year = np.sum([w0*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                                    w1*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                                    w0[-1],w1[-1]]@tendist[1:5,:52])/np.sum(tendist[1:5,:52])

    mean_wages_third_year = np.sum([w0*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                                    w1*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                                    w0[-1],w1[-1]]@tendist[1:5,104:156])/np.sum(tendist[1:5,104:156])

    mean_wages_fifth_year = np.sum([w0*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                                    w1*H_pi_dist@np.transpose(pigrid>pibar)/np.sum(H_pi_dist[pigrid>pibar]),                                    w0[-1],w1[-1]]@tendist[1:5,208:260])/np.sum(tendist[1:5,208:260])
...
def solvemodel(x,targs,vers,show):
    ...
    while abs(difpe) > tol:
        ...
        while difs > tol:
            ...
            S0new = np.maximum(pigrid*yg+(1-pigrid)*yb-b-localkh                               +beta*(1-lam)*((1-pkh)*(alph*pigrid*S0old[-1]+(1-alph)*S0old)                                              +pkh*(alph*pigrid*S1old[-1]+(1-alph)*S1old))                               -beta*pe*eta*np.dot(H_pi_dist,S0old),0)
            # Array of total surplus when not paying startup cost (p.277) for each pi
            S1new = np.maximum(pigrid*yg+(1-pigrid)*yb-b                               +beta*(1-lam)*(alph*pigrid*S1old[-1]+(1-alph)*S1old)                               -beta*pe*eta*np.dot(H_pi_dist,S0old),0)
    ...
    fqseps = 13*np.sum(tendist[1:3,0]) - np.sum(tendist[1:3,13]) - np.sum(tendist[1:3,12]) -            np.sum(tendist[1:3,11]) - np.sum(tendist[1:3,10]) - np.sum(tendist[1:3,9]) -            np.sum(tendist[1:3,8]) - np.sum(tendist[1:3,7]) - np.sum(tendist[1:3,6]) -            np.sum(tendist[1:3,5]) - np.sum(tendist[1:3,4]) - np.sum(tendist[1:3,3]) -            np.sum(tendist[1:3,2]) - np.sum(tendist[1:3,1]);
...
targets = np.array([0.419, 0.486, 22.7/7])  
...
targets = np.array([0.419, 0.486, 22.7/7])  # Targeted values for probg, jfr, and meanvacdur (1999)
