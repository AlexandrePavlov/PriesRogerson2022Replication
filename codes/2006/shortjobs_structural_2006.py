...
startvals = np.array([0.5, 0.5, 0.5]) 

# Targeted values for probg, jfr, and meanvacdur (2006)

# probability of good match taken from shortjobs_reduced_form
# job finding rate calculated from BLS data (values provided by authors)
# mean vacancy duration taken from DHI-DFH data available at FRED and divided by 7 (21.57783 is mean of 2006 values)
targets = np.array([0.459, 0.410, 21.57783/7])  

Params2006 = fsolve(lambda x: solvemodel(x,targets,1,0), startvals, xtol=1e-12)
fval = solvemodel(Params2006,targets,1,0)

# Reveal the calibrated 2006 values of sigeps, A, and kr
...
# Table 4: The model's 2006 moments are calculated and displayed here

# 2006 values of model moments
print('2006 values of model moments')
stats2006 = [oneqhazrate, twoqhazrate, threeplushazrate, hiresrate, jfr, meanvacdur, u]
print(stats2006)
print('\n')
...
alph = 0.1276
lam = 0.0071
targets = np.array([0.486, 0.364, 28.1/7])  # Targeted values for probg, jfr, and meanvacdur (2017)
...
alph = 0.1405  # Set alph back to its 2006 value
lam = 0.0078  # Set lam back to its 2006 value

# Change kr
solvemodel([Params2006[0],Params2006[1],Params2017a[2]],targets,1,0)

stats2017 = [oneqhazrate, twoqhazrate, threeplushazrate, hiresrate, jfr, meanvacdur, u]
print('Changes due to change in kr:')
x = np.array(stats2017) - np.array(stats2006)
print([round(elem,4) for elem in x])
print('\n')

# Change A
solvemodel([Params2006[0],Params2017a[1],Params2006[2]],targets,1,0)
...
# Change lam
lam = 0.0071
solvemodel(Params2006,targets,1,0)
lam = 0.0078  # Restore to 2006 value
...
lam = 0.0078
solvemodel(Params2006,targets,1,0)
alph = 0.1405  # Restore to 2006 value
...
# Change sigeps
solvemodel([Params2017a[0],Params2006[1],Params2006[2]],targets,1,0)
...
# Note that there is no need here to re-do anything for the 2006 calibration. These exercises are just re-doing the 2017 calibration under the assumption that now pi0, rather than sigeps, is allowed to vary.

sigeps = 0.8088  # Fix this at the 2006 calibrated value
kh = 0.11
...
# Note: Here the first parameter is pi0, so need to set it to its 2006 value of 0.4

# Change kr
solvemodel([0.4,Params2006[1],Params2017b[2]],targets,2,0)
...
# Change A
solvemodel([0.4,Params2017b[1],Params2006[2]],targets,2,0)
...
# Change lam
lam = 0.0071
solvemodel([0.4,Params2006[1],Params2006[2]],targets,2,0)
lam = 0.0078  # Restore to 2006 value
...
# Change pi0
solvemodel([Params2017b[0],Params2006[1],Params2006[2]],targets,2,0)
...
pi0 = 0.4
kr = 0.8141  # Fix this at the 2006 calibrated value

# 2017 calibration
alph = 0.1276
lam = 0.0071
targets = np.array([0.486, 0.364, 28.1/7])
...
# Note: Here the third parameter is kh, so need to set it to its 2006 value of 0.11 (except for the exercise where it is being changed)

# Change kh
solvemodel([Params2006[0],Params2006[1],Params2017c[2]],targets,3,0)
...
# Change A
solvemodel([Params2006[0],Params2017c[1],0.11],targets,3,0)
...
# Change lam
lam = 0.0071
solvemodel([Params2006[0],Params2006[1],0.11],targets,3,0,)
lam = 0.0078  # Restore to 2006 value
...
# Change sigeps
solvemodel([Params2017c[0],Params2006[1],0.11],targets,3,0)
...
targets = np.array([0.459, 0.410, 21.5/7])  # Targeted values for probg, jfr, and meanvacdur (2006)

Params2006b = fsolve(lambda x: solvemodel(x,targets,1,0), startvals, xtol=1e-12)
fval = solvemodel(Params2006b,targets,1,0)
...
# Change kr
solvemodel([Params2006b[0],Params2006b[1],Params2017d[2]],targets,1,0)
...
# Change A
solvemodel([Params2006b[0],Params2017d[1],Params2006b[2]],targets,1,0)
...
# Change lam
lam = 0.0071
solvemodel(Params2006b,targets,1,0)
lam = 0.0078  # Restore to 2006 value
...
# Change sigeps
solvemodel([Params2017d[0],Params2006b[1],Params2006b[2]],targets,1,0)
...
q1wages2006 = mean_wages_first_qtr
y5wages2006 = mean_wages_fifth_year
...
print(q1wages2017/q1wages2006-1)
print('\n')

print('Percentage change in first-quarter/fifth-year ratio')
print((q1wages2017/y5wages2017)/(q1wages2006/y5wages2006)-1)
print('\n')
...
# 2017 value for sigeps, 2006 for all others
alph = 0.1405
lam = 0.0078
solvemodel([Params2017a[0],Params2006[1],Params2006[2]],targets,1,0)
calculate_mean_wages()
...
print('First-quarter wages in 2006')
print(q1wages2006)
print('\n')
...
print('Percentage change in first-quarter mean wages')
print(round(q1wages2017,3)/round(q1wages2006,3)-1)
print('\n')

print('Fifth-year wages in 2006')
print(round(y5wages2006,3))
print('\n')
...
print(round(y5wages2017,3)/round(y5wages2006,3)-1)
print('\n')
