...
data_sa <- read.csv(paste(wd,"qwisa_2006.csv",sep="/"))

#drop if sex == 0 (i.e. keep dataframe with sex != 0)
...
#egen cell=group(sex agegrp year quarter)
data_sa <- data_sa %>%
  mutate(cell = group_indices(., sex, agegrp, year, quarter))
...
#egen cellid=group(sex agegrp)
data_sa <- data_sa %>%
  mutate(cellid = group_indices(., sex, agegrp))
...
#hires as a fraction of total count of employment spells in the quarter
data_sa$hirerate=data_sa$allhira/data_sa$allemptot

#drop if time<=2005*4+4 | time>2018*4

#can't compare with factors so I need to transform as numeric
#7961 --> 1
#2005*4+4=8024 --> 64
#2018*4=8072 --> 112
#hence condition should be to drop if time<=64 | time>112
data_sa <- data_sa[!(as.numeric(data_sa$time) <= 64 | as.numeric(data_sa$time) > 112), ]
...
#sort cell date
data_sa <- arrange(data_sa,cell,date)
...
write.csv(data_sa,file="shortjobs_sex_age_2006.csv")
...
#figure 4
data_male <- data_sa[data_sa$sex == 1, ]
data_female <- data_sa[data_sa$sex == 2, ]

age_sex_label <- c("14-18","19-21","22-24","25-34","35-44","45-54","55-64",
                   "65-99","14-18","19-21","22-24","25-34","35-44","45-54",
                   "55-64","65-99")

ggplot() +
  geom_point(data = data_male, aes(x = oneqsepsratesa_first, y = oneqsepsratesa_last), color = "red") +
  geom_point(data = data_female, aes(x = oneqsepsratesa_first, y = oneqsepsratesa_last), color = "blue") +
  geom_abline(slope = 1, intercept = 0, color = "black") +
  xlim(0, 0.3) +
  ylim(0, 0.3) +
  labs(x = expression("2006 One quarter incidence rate (q"[1]*")"),
       y = expression("2017 One quarter incidence rate (q"[1]*")")) +
  geom_text(data = data_sa,aes(x = oneqsepsratesa_first, y = oneqsepsratesa_last,label=age_sex_label),nudge_x=0.005,nudge_y=0.005) +
  theme(panel.background = element_rect(fill = 'white', colour = 'white'))

ggsave('oneqseps_firstlast_sex_age_2006.pdf')

ggplot() +
  geom_point(data = data_male, aes(x = hireratesa_first, y = hireratesa_last), color = "red") +
  geom_point(data = data_female, aes(x = hireratesa_first, y = hireratesa_last), color = "blue") +
  geom_abline(slope = 1, intercept = 0, color = "black") +
  xlim(0.08, 0.51) +
  ylim(0.08, 0.51) +
  labs(x = "2006 Hires rate (h)",
       y = "2017 Hires rate (h)") +
  geom_text(data = data_sa,aes(x = hireratesa_first, y = hireratesa_last,label=age_sex_label),nudge_x=0.005,nudge_y=0.005) +
  theme(panel.background = element_rect(fill = 'white', colour = 'white'))

ggsave('hirerate_firstlast_sex_age_2006.pdf')